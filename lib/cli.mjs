...
import { createStoryboard, loadStoryboard } from './storyboard.mjs';
...
async function commandCreate(positional, flags) {
  ...
  const storyboard = createStoryboard(topic, {
    slides: parseNumberFlag(flags.slides, 6),
    language: flags.language || flags.lang || 'zh-TW'
  });
  await writeJson(out, storyboard);
  console.log(`Created storyboard: ${out}`);
}
...
async function commandRender(flags) {
  ...
  const storyboard = await loadStoryboard(resolve(process.cwd(), input));
...
Usage:
  autocut create "AI Agent 技術提案" --slides 6 --out storyboard.json
