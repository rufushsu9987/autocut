...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* Beat timeline: content reveals and emphasis remain active throughout a slide. */
    .beat-managed [data-beat-target],
    .beat-managed [data-beat-title] {
      animation: none !important;
      opacity: 0.18;
      transform: translateY(18px) scale(0.985);
      filter: saturate(0.55);
      transition: opacity 420ms ease, transform 420ms cubic-bezier(.2,.8,.2,1), filter 420ms ease, box-shadow 420ms ease, border-color 420ms ease, background 420ms ease;
    }

    .beat-managed [data-beat-target].beat-visible,
    .beat-managed [data-beat-title].beat-visible,
    .beat-managed.beat-complete [data-beat-target],
    .beat-managed.beat-complete [data-beat-title] {
      opacity: 1;
      transform: none;
      filter: none;
    }

    .beat-managed [data-beat-target].beat-focus {
      opacity: 1;
      transform: translateY(-8px) scale(1.025);
      filter: none;
      border-color: color-mix(in srgb, var(--accent) 72%, var(--line));
      box-shadow: 0 0 0 4px color-mix(in srgb, var(--accent) 16%, transparent), 0 24px 70px color-mix(in srgb, var(--accent) 18%, transparent);
    }
    ...
    /* Infographic: simple problem -> method -> result diagram. */
    .infographic-stage {
      grid-template-rows: auto minmax(0, 1fr) auto;
    ...
    .diagram-node {
      ...
      border: 1.5px solid color-mix(in srgb, var(--fg) 34%, var(--line));
    ...
    .diagram-node.problem { background: color-mix(in srgb, var(--panel) 88%, transparent); }
    .diagram-node.method { background: linear-gradient(145deg, color-mix(in srgb, var(--accent) 13%, var(--panel-strong)), var(--panel-strong)); }
    .diagram-node.result { background: linear-gradient(145deg, color-mix(in srgb, var(--accent-2) 10%, var(--panel-strong)), var(--panel-strong)); }
    .diagram-node::after { content: ""; position: absolute; width: 130px; height: 130px; right: -66px; bottom: -68px; border: 1px solid color-mix(in srgb, var(--accent) 30%, transparent); border-radius: 50%; box-shadow: 0 0 0 18px color-mix(in srgb, var(--accent) 5%, transparent); }
    .diagram-node-header { position: relative; z-index: 2; display: flex; align-items: center; justify-content: space-between; gap: 12px; font-family: var(--mono-font); font-size: 14px; letter-spacing: 0.12em; text-transform: uppercase; color: var(--muted); }
    .diagram-icon { display: inline-grid; place-items: center; width: 42px; height: 42px; border: 1.5px solid var(--fg); border-radius: 50%; color: var(--accent); font-family: var(--heading-font); font-size: 23px; font-weight: 800; }
    .diagram-art { position: relative; z-index: 2; display: block; width: 100%; height: clamp(90px, 10vh, 148px); object-fit: contain; object-position: center; margin: 2px 0 0; }
    .diagram-title { position: relative; z-index: 2; margin: auto 0 0; font-family: var(--heading-font); font-size: clamp(31px, 2.9vw, 57px); line-height: 0.98; letter-spacing: -0.045em; }
    .diagram-value { position: relative; z-index: 2; margin: 4px 0 0; font-family: var(--heading-font); font-size: clamp(52px, 5.1vw, 96px); line-height: 0.88; color: var(--accent); letter-spacing: -0.06em; }
    .diagram-detail { position: relative; z-index: 2; color: var(--muted); font-size: clamp(17px, 1.12vw, 23px); line-height: 1.35; }
    .diagram-list { position: relative; z-index: 2; display: grid; gap: 8px; margin: auto 0 0; padding: 0; list-style: none; }
    .diagram-list li { padding: 11px 12px; border: 1px solid var(--line); border-radius: 11px; background: color-mix(in srgb, var(--panel-strong) 74%, transparent); font-size: clamp(16px, 1vw, 21px); font-weight: 700; }
    .diagram-list li::before { content: "✓"; display: inline-block; margin-right: 9px; color: var(--accent-2); font-family: var(--mono-font); }
    .diagram-status { position: relative; z-index: 2; margin-top: auto; font-family: var(--mono-font); font-size: 14px; color: var(--muted); letter-spacing: 0.08em; text-transform: uppercase; }
    .diagram-progress { position: relative; z-index: 2; height: 11px; border: 1.5px solid var(--fg); border-radius: 999px; padding: 2px; }
    .diagram-progress span { display: block; width: 72%; height: 100%; border-radius: inherit; background: var(--fg); }
    .diagram-arrow { position: relative; z-index: 3; color: var(--accent); font-family: var(--mono-font); font-size: clamp(28px, 2.6vw, 50px); line-height: 1; }
    .takeaway-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(210px, 1fr)); gap: 12px; }
    .takeaway-pill { min-width: 0; padding: 13px 18px; border: 1.5px solid color-mix(in srgb, var(--fg) 30%, var(--line)); border-radius: 999px; background: color-mix(in srgb, var(--panel-strong) 84%, transparent); text-align: center; font-size: clamp(16px, 1vw, 21px); font-weight: 700; }
    ...
    @media (max-aspect-ratio: 4/3) {
      ...
      .infographic-visual { grid-template-columns: 1fr; }
  ...
  <script>
    (() => {
      ...
      function clearBeatTimers() {
        beatTimers.forEach((timer) => clearTimeout(timer));
        beatTimers = [];
      }

      function resetBeatState(slide) {
        slide.classList.remove('beat-managed', 'beat-complete');
        slide.querySelectorAll('.beat-visible, .beat-focus').forEach((element) => {
          element.classList.remove('beat-visible', 'beat-focus');
        });
      }

      function beatTargets(slide, target) {
        const targets = [...slide.querySelectorAll('[data-beat-target]')];
        if (target == null) return targets;
        return targets.filter((element) => element.dataset.beatTarget === String(target));
      }

      function runBeat(slide, beat) {
        const targets = beatTargets(slide, beat.target);
        if (beat.action === 'show-title') {
          slide.querySelectorAll('[data-beat-title]').forEach((element) => element.classList.add('beat-visible'));
          return;
        }
        if (['reveal', 'reveal-step'].includes(beat.action)) {
          targets.forEach((element) => element.classList.add('beat-visible'));
          return;
        }
        if (['focus', 'focus-step', 'emphasize'].includes(beat.action)) {
          slide.querySelectorAll('.beat-focus').forEach((element) => element.classList.remove('beat-focus'));
          targets.forEach((element) => element.classList.add('beat-visible', 'beat-focus'));
          return;
        }
        if (['complete', 'complete-flow'].includes(beat.action)) {
          slide.classList.add('beat-complete');
          slide.querySelectorAll('.beat-focus').forEach((element) => element.classList.remove('beat-focus'));
        }
      }

      function scheduleBeats(slide) {
        const raw = slide.dataset.beats;
        if (!raw) return;
        let beats;
        try {
        ...
        slide.classList.add('beat-managed');
        beats.forEach((beat) => {
          const at = Math.max(0, Number(beat.at) || 0) * 1000;
          const run = () => runBeat(slide, beat);
          if (at === 0) run();
          else beatTimers.push(setTimeout(run, at));
        });
      }
      ...
      function show(index) {
        ...
        slides.forEach((slide) => {
          ...
          resetBeatState(slide);
        });
        current = next;
        void slides[next].offsetWidth;
        slides[next].classList.add('active');
        scheduleBeats(slides[next]);
...
function renderSlide(slide, index, total, storyboard) {
  ...
  const beatData = slide.beats.length ? ` data-beats="${escapeHtml(JSON.stringify(slide.beats))}"` : '';

  return `    <section class="slide layout-${escapeHtml(slide.layout)} effect-${escapeHtml(slide.effect)} variant-${escapeHtml(slide.variant)}" id="${escapeHtml(slide.id)}" data-index="${index}" data-layout="${escapeHtml(slide.layout)}" data-effect="${escapeHtml(slide.effect)}"${beatData}>
...
function renderHero(slide, index, total, storyboard) {
  return `<div class="layout-stage hero-stage">
          <div class="hero-copy">
            ...
            <h1 class="slide-title hero-title reveal" data-beat-title style="--delay:140ms">${escapeHtml(slide.title)}</h1>
...
function renderSection(slide, index) {
  return `<div class="layout-stage section-stage">
          ...
          <div class="section-copy">
            ...
            <h1 class="slide-title reveal" data-beat-title style="--delay:240ms">${escapeHtml(slide.title)}</h1>
...
function renderSplit(slide) {
  return `<div class="layout-stage split-stage">
          <div class="split-copy">
            ${renderKicker(slide, 60)}
            <h1 class="slide-title reveal" data-beat-title style="--delay:150ms">${escapeHtml(slide.title)}</h1>
...
function renderFlow(slide) {
  ...
  const cards = steps.map((step, index) => `<article class="flow-step reveal" data-step="${String(index + 1).padStart(2, '0')}" data-beat-target="${index}" style="--delay:${180 + index * 105}ms">
...
function renderInfographic(slide) {
  const infographic = slide.infographic || {
    left: { title: 'Problem', detail: '' },
    center: { title: 'Method', detail: '' },
    right: { title: 'Result', detail: '', items: [] },
    takeaways: []
  };
  const takeaways = infographic.takeaways || [];
  return `<div class="layout-stage infographic-stage">
          ${renderLayoutHeading(slide)}
          <div class="infographic-visual">
            ${renderInfographicNode(infographic.left, 'problem', 'PROBLEM', '!', 'left', 150)}
            <div class="diagram-arrow reveal" data-beat-target="arrow-left" style="--delay:260ms" aria-hidden="true">→</div>
            ${renderInfographicNode(infographic.center, 'method', 'METHOD', '→', 'center', 300)}
            <div class="diagram-arrow reveal" data-beat-target="arrow-right" style="--delay:400ms" aria-hidden="true">→</div>
            ${renderInfographicNode(infographic.right, 'result', 'RESULT', '✓', 'right', 450)}
          </div>
          ${takeaways.length ? `<div class="takeaway-row">${takeaways.map((takeaway, index) => `<span class="takeaway-pill reveal" data-beat-target="takeaway-${index}" style="--delay:${560 + index * 80}ms">${escapeHtml(takeaway)}</span>`).join('')}</div>` : ''}
        </div>`;
}

function renderInfographicNode(node, role, label, fallbackIcon, target, delay) {
  const items = node.items || [];
  return `<article class="diagram-node ${role} reveal" data-beat-target="${target}" style="--delay:${delay}ms">
            <div class="diagram-node-header"><span>${escapeHtml(label)}</span><span class="diagram-icon">${escapeHtml(node.icon || fallbackIcon)}</span></div>
            ${node.art ? `<img class="diagram-art" src="${escapeHtml(node.art)}" alt="" aria-hidden="true" />` : ''}
            ${node.value ? `<div class="diagram-value">${escapeHtml(node.value)}</div>` : ''}
            <h2 class="diagram-title">${escapeHtml(node.title)}</h2>
            ${node.detail ? `<div class="diagram-detail">${escapeHtml(node.detail)}</div>` : ''}
            ${items.length ? `<ul class="diagram-list">${items.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul>` : ''}
            ${role === 'result' ? `<div class="diagram-status">${escapeHtml(node.status || '進度可見')}</div><div class="diagram-progress" aria-hidden="true"><span></span></div>` : ''}
          </article>`;
}
...
function renderMetrics(slide) {
  ...
  const cards = metrics.map((metric, index) => `<article class="metric-card reveal" data-beat-target="${index}" style="--delay:${170 + index * 110}ms;--meter:${0.48 + ((index * 17) % 42) / 100}">
...
function renderCompareSide(side, sideClass, symbol, delay) {
  return `<article class="compare-side ${sideClass} reveal" data-symbol="${symbol}" data-beat-target="${sideClass}" style="--delay:${delay}ms">
...
function renderQuote(slide) {
  ...
  return `<div class="layout-stage quote-stage">
          <div class="quote-main">
            ...
            <blockquote class="quote-text reveal" data-beat-title style="--delay:170ms">${escapeHtml(quote)}</blockquote>
...
function renderTimeline(slide) {
  ...
  const items = slide.timeline.map((item, index) => `<li class="timeline-item reveal" data-beat-target="${index}" style="--delay:${180 + index * 120}ms">
...
function renderCards(slide) {
  ...
  const items = cards.map((card, index) => `<article class="content-card reveal" data-beat-target="${index}" style="--delay:${170 + index * 105}ms">
...
function renderStatement(slide) {
  ...
  return `<div class="layout-stage statement-stage">
          <div class="statement-wrap">
            ${renderKicker(slide, 60)}
            ${slide.emphasis ? `<div class="statement-emphasis reveal" data-beat-target="emphasis" style="--delay:130ms">${escapeHtml(slide.emphasis)}</div>` : ''}
            <h1 class="statement-text reveal" data-beat-title style="--delay:220ms">${escapeHtml(statement)}</h1>
...
function renderCode(slide) {
  ...
  const lines = code.content.split('\n').map((line, index) => {
    const className = code.highlights.includes(index + 1) ? 'code-line highlight' : 'code-line';
    return `<span class="${className}" data-beat-target="${index}">${escapeHtml(line || ' ')}</span>`;
  }).join('');
...
function renderArchitecture(slide) {
  ...
  const items = layers.map((layer, index) => `<article class="architecture-layer reveal" data-beat-target="${index}" style="--delay:${170 + index * 110}ms">
...
function renderLayoutHeading(slide) {
  return `<header class="layout-heading">
          <div>${renderKicker(slide, 50)}<h1 class="slide-title reveal" data-beat-title style="--delay:130ms">${escapeHtml(slide.title)}</h1>${slide.subtitle ? `<div class="slide-subtitle reveal" style="--delay:230ms">${escapeHtml(slide.subtitle)}</div>` : ''}</div>
...
function renderBullets(bullets, startDelay = 0) {
  ...
  return `<ul class="bullet-list">${bullets.map((bullet, index) => `<li class="reveal" data-beat-target="${index}" style="--delay:${startDelay + index * 85}ms">${escapeHtml(bullet)}</li>`).join('')}</ul>`;
}
